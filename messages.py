...
'''
    ...
    class Santa:
        def __init__(self):
            self.on_start_1 = '''Коротко про правила, расчитываем на человека, который впервые про это слышит. Катя писала в конфе, что должно быть в приветственном сообщении, исходи из этого'''
            self.on_start_2 = '''Чтобы отказаться от участия, отправь команду /stop_santa'''
            self.ask_wishes = '''Давай начнем, напиши пожелания по подарку'''
            self.ask_address = '''Напиши свой адрес'''
            self.ask_meeting = '''Будешь ли ты на встрече?'''
            self.on_end = '''Ок, ты добавлен в бд и примешь участие в жеребьевке. Если возникнут вопросы, пиши (((кому-то)))'''
